...
% cscbme/张明
% 利用谱减法和方差法进行端点检测 
function [voiceseg,vsl,time,output,frameTime,Dvarm,T1,T2,fn] = basicspectral_var(array1D,fs,alg) 
...
a=3; b=0.01;
output=simplesubspec(x,wlen,inc,NIS,a,b);  % 计算谱减

y=enframe(output,wnd,inc)';                  % 分帧
fn=size(y,2);                           % 求帧数
...
T1=1*dth;
T2=1*dth;
[voiceseg,vsl,SF,NF,T1,T2]=vad_param1D(Dvarm,T1,T2,alg);% 频域方差双门限的端点检测
% 作图
% figure(1)
% clf

% 2017/12/1
% cscbme/张明
% 利用短时能量和过零率反向比较进行端点检测 
function [voiceseg,vsl,time,xx,frameTime,ampm,zcrm,T1,T2,T3,fn] = ez_revr(array1D,fs,alg)
...
[voiceseg,vsl,T1,T2,T3,SF,NF]=vad_param2D_revr(alg,amp,zcr,amp2,amp1,zcr2);% 端点检测
% 作图
% subplot 211; 
% plot(time,xx,'k');
